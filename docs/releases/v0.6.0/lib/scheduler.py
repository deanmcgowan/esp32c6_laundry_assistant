...
def _fmt_stockholm_hm(utc_mp_epoch):
    t = utc_to_stockholm_tuple(int(utc_mp_epoch))
...
def compute_recommendations(prices_cache, now_utc_mp):
    """
    Returns dict suitable for /api/recommendations.
    Uses spot prices only; assumes default cycle kWh values.
    """
    slots = flatten_slots(prices_cache)
    cur_price = price_at_utc(slots, now_utc_mp)
    ...
    step_s = 900  # align to 15-min boundaries
            ...
            results.append(
                {
                    ...
                    "recommended_start_local_hm": None,
        ...
        results.append(
            {
                ...
                "recommended_start_local_hm": _fmt_stockholm_hm(best["start"]),
