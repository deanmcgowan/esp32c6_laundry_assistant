...
def _html_index():
    # Minimal HTML+CSS+JS. No external assets.
    html = r"""<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
<div class="wrap">
  ...
  <div id="tabAppliances" class="hidden">
    <div class="grid">
      <div class="card">
        ...
        <div class="okmsg">
          Savings are estimates using default kWh assumptions per cycle (can be made configurable later).
        </div>
  ...
  <div class="foot">
    Tip: <span class="mono">/api/status</span>, <span class="mono">/api/prices</span>, <span class="mono">/api/recommendations</span>
  </div>
</div>
...
function fmt(x, digits=3){
  ...
  if(typeof x === 'number') return x.toFixed(digits).replace(/\.000$/, '');
  return String(x);
}

function fmtMoney(x){
  if(x === null || x === undefined) return '—';
  return (typeof x === 'number') ? (x.toFixed(2) + ' SEK') : String(x);
}
...
async function refreshOnce(){
  try{
    ...
    // Ngenic
    const n = s.ngenic || {};
    document.getElementById('imp').textContent = fmt(n.import_kW, 3);
    document.getElementById('exp').textContent = fmt(n.export_kW, 3);
    document.getElementById('net').textContent = fmt(n.net_kW, 3);
    document.getElementById('age').textContent = fmt(n.age_s, 0);
    document.getElementById('intv').textContent = fmt(n.learned_interval_s, 1);
    ...
    if(age === null || age === undefined){
      dotState('warn');
      document.getElementById('freshText').textContent = 'Ngenic: unknown age';
    } else if(age <= 30){
      dotState('ok');
      document.getElementById('freshText').textContent = 'Ngenic: fresh (' + age + 's)';
    } else if(age <= 120){
      dotState('warn');
      document.getElementById('freshText').textContent = 'Ngenic: stale (' + age + 's)';
    } else {
      dotState('bad');
      document.getElementById('freshText').textContent = 'Ngenic: very stale (' + age + 's)';
    }
  }catch(e){
    setErr('ngenicErr', String(e));
  }

  // Prices (separate call)
  try{
    const rp = await fetch('/api/prices', { cache: 'no-store' });
    const p = await rp.json();

    document.getElementById('pricesAge').textContent = fmt(p.age_s, 0);
    ...
    // Only show an error if it isn't a normal "tomorrow not ready" situation.
    const err = p.last_error;
    setErr('pricesErr', err ? String(err) : '');
  }catch(e){
    setErr('pricesErr', String(e));
  }

  // Recommendations (only needed for appliances tab, but cheap enough)
  try{
    const rr = await fetch('/api/recommendations', { cache: 'no-store' });
    const r = await rr.json();

    document.getElementById('spotNow').textContent = fmt(r.current_spot_sek_per_kwh, 5);
      ...
      tbody.innerHTML = rows.map(x => {
        return `
          <tr>
            ...
            <td class="mono">${fmt(x.avg_price_now_sek_per_kwh, 5)} SEK/kWh</td>
            <td class="mono">${fmt(x.avg_price_recommended_sek_per_kwh, 5)} SEK/kWh</td>
            <td class="mono">${fmtMoney(x.estimated_saving_sek)}</td>
          </tr>
        `;
      }).join('');
    }
