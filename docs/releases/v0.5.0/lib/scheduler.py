...
def compute_recommendations(prices_cache, now_utc):
    ...
    # Align start times to 15-minute boundaries (prices are now often 15-min granularity).
    # 900s = 15 minutes
    step_s = 900
    ...
    for a in appliances:
        ...
        # Baseline: "run now" (delay 0), aligned to next slot boundary
        ...
        if best is None or base_avg is None:
            results.append({
                ...
                "avg_price_now_sek_per_kwh": base_avg,
        ...
        results.append({
            ...
            "avg_price_now_sek_per_kwh": base_avg,
            "avg_price_recommended_sek_per_kwh": best["avgp"],
            "estimated_saving_sek": saving,
            "assumed_kwh_per_cycle": assumed_kwh,
        })

    return {
        "current_spot_sek_per_kwh": cur_price,
        "appliances": results,
        "error": None,
    }