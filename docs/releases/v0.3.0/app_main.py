...
def sync_time():
    # Ensure Wi‑Fi is up, then set UTC time
    wifi_up()
    ntptime.settime()
...
def main():
    ...
    # Bring Wi‑Fi up and print IP (requested)
    ip = None
    try:
        ip = wifi_up()
    ...
    try:
        while True:
            # Keep Ngenic updated (adaptive; cheap when idle)
            ng.refresh_if_due()
            ...
            # Serve one request if present
            srv.poll_once(get_status)
